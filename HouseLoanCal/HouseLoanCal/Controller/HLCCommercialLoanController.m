...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [MobClick beginLogPageView:@"PageCommer"];
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"PageCommer"];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    switch (section) {
        case kHLCLoanSectionInput: {
            numOfRows = kHLCCommercialInputCount + 1;
        }
            break;
        case kHLCLoanSectionOutputSummary: {
            if (self.isShowOutput) {
                numOfRows = kHLCCommercialSummaryCount + 1;
            }
        }
            break;
        ...
        case kHLCLoanSectionInput: {
            switch (row) {
                ...
                case kHLCCommercialInputPeriod: {
                    ...
                    HLCLoanInputTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
                    if (!cell) {
                        cell = [[HLCLoanInputTableViewCell alloc] initWithHLCStyle:HLCLoanInputTableViewCellStylePickerView reuseIdentifier:cellIdentifier withTag:2001];
                    }
            ...
            if (self.isShowOutput) {
                switch (row) {
                    ...
                    case kHLCCommercialSummaryAccuMoney: {
                        ...
                        break;
                    case kHLCCommercialSummaryCount: {
...
- (void)resetButtonDidClick:(UIButton *)button {
    [MobClick event:kHLCCommerResetClick];
    [self resetAndReloadData];
}

// 计算按钮被点击后的操作
- (void)calculateButtonDidClick:(UIButton *)button {
    [MobClick event:kHLCCommerCalClick];
